...
export class CitreaAPI {
  private baseURL = 'https://explorer.testnet.citrea.xyz/api'
  private coingeckoAPI = 'https://api.coingecko.com/api/v3'
  ...
  /**
   * Get complete portfolio data - FAST VERSION
   */
  async getPortfolioData(address: string): Promise<PortfolioData> {
    console.log(`Getting portfolio data for ${address}`)
    
    // Parallel API calls to Citrea explorer (much faster than RPC)
    ...
    const totalTokenValue = tokenBalances.reduce((sum, token) => sum + token.balanceUSD, 0)
    
    return {
      address,
      totalValueUSD: balance.balanceUSD + totalTokenValue,
      ethBalance: balance.balance,
      ethBalanceUSD: balance.balanceUSD,
      tokens: tokenBalances,
      transactions,
      transactionCount: transactions.length,
      performance24h: 0, // Could be calculated from transaction history
      lastUpdated: Date.now()
    }
  }

  /**
   * Get ETH balance using Citrea API (much faster than RPC)
   */
  public async getBalance(address: string): Promise<{ balance: string; balanceUSD: number }> {
    try {
      ...
      // Get BTC price (since Citrea uses BTC as base)
      const btcPrice = await this.getBTCPrice()
      const balanceUSD = parseFloat(balanceETH) * btcPrice
  ...
  /**
   * Get transaction history using Citrea API - MUCH FASTER than RPC or subgraph
   */
  async getTransactionHistory(address: string, limit = 20): Promise<Transaction[]> {
    try {
      // Get regular transactions
      const txUrl = `${this.baseURL}?module=account&action=txlist&address=${address}&sort=desc&page=1&offset=${limit}`
      
      // Get token transactions  
      const tokenTxUrl = `${this.baseURL}?module=account&action=tokentx&address=${address}&sort=desc&page=1&offset=${limit}`
      ...
      // Process regular ETH transactions
      if (txData.status === '1' && txData.result) {
        for (const tx of txData.result) {
          ...
          transactions.push({
            ...
            to: tx.to,
            value: tx.value,
            valueUSD: valueETH, // You could multiply by BTC price here
            gasUsed: tx.gasUsed,
      ...
      if (tokenTxData.status === '1' && tokenTxData.result) {
        for (const tx of tokenTxData.result) {
          ...
          transactions.push({
            ...
            to: tx.to,
            value: tx.value,
            valueUSD: 0, // Would need token prices
            gasUsed: tx.gasUsed,
            gasPrice: tx.gasPrice,
            timestamp: parseInt(tx.timeStamp),
            blockNumber: parseInt(tx.blockNumber),
  ...
  /**
   * Get all token balances using Citrea API - MUCH SIMPLER
   */
  async getTokenBalances(address: string): Promise<TokenBalance[]> {
    try {
      ...
      for (const token of data.result) {
        ...
        if (parseInt(token.balance) === 0) continue
        ...
        tokenBalances.push({
          token: {
            ...
            priceUSD: 0 // You could fetch token prices here
          },
          balance: token.balance,
          balanceUSD: 0 // Would need token USD prices
        })
      }
  ...
  async getTokenBalance(address: string, contractAddress: string): Promise<number> {
    try {
      const url = `${this.baseURL}?module=account&action=tokenbalance&contractaddress=${contractAddress}&address=${address}`
      const response = await fetch(url)
      const data = await response.json()
  ...
  async getTokenInfo(contractAddress: string): Promise<TokenInfo | null> {
    try {
      const url = `${this.baseURL}?module=token&action=getToken&contractaddress=${contractAddress}`
      const response = await fetch(url)
      const data = await response.json()
  ...
  async healthCheck(): Promise<{ isHealthy: boolean; latestBlock: number }> {
    try {
      const url = `${this.baseURL}?module=block&action=eth_block_number`
      const response = await fetch(url)
      const data = await response.json()
      
      return {
        isHealthy: data.result ? true : false,
        latestBlock: data.result ? parseInt(data.result, 16) : 0
      }
    } catch (error) {
      console.error('Health check failed:', error)
      return { isHealthy: false, latestBlock: 0 }
    }
  }
