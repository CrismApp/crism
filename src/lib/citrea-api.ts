...
import type { TokenBalance, Transaction, PortfolioData } from './goldsky/client'
...
export class CitreaAPI {
  ...
  /**
   * Enhanced portfolio data fetching with Goldsky fallback
   * Tries Goldsky first for instant results, falls back to RPC if needed
   */
  async getPortfolioData(address: string): Promise<PortfolioData> {
    console.log(`Getting enhanced portfolio data for ${address}`)
    
    try {
      // Try Goldsky first for instant results
      if (goldskyClient.isAvailable()) {
        console.log('Attempting Goldsky fetch...')
        const goldskyData = await goldskyClient.getPortfolioData(address)
        
        if (goldskyData && goldskyData.transactionCount > 0) {
          console.log(`✅ Goldsky data found: ${goldskyData.transactionCount} txs, ${goldskyData.tokens.length} tokens`)
          return goldskyData
        } else {
          console.log('⚠️ Goldsky returned empty data, falling back to RPC')
        }
      } else {
        console.log('📡 Goldsky not available, using RPC method')
      }
      
      // Fallback to your existing RPC method
      console.log('🔄 Fetching via RPC...')
      return await this.getRPCPortfolioData(address)
      
    } catch (error) {
      console.error('Enhanced portfolio fetch error:', error)
      // Always fallback to RPC on any error
      return await this.getRPCPortfolioData(address)
    }
  }

  /**
   * Enhanced transaction history with Goldsky optimization
   */
  async getTransactionHistory(address: string, limit = 20): Promise<Transaction[]> {
    try {
      // Try Goldsky first
      if (goldskyClient.isAvailable()) {
        const goldskyTxs = await goldskyClient.getTransactionHistory(address, limit)
        if (goldskyTxs.length > 0) {
          console.log(`✅ Goldsky found ${goldskyTxs.length} transactions`)
          return goldskyTxs
        }
      }
      
      // Fallback to your existing optimized RPC method
      return await this.getRPCTransactionHistory(address, limit)
    } catch (error) {
  ...
  /**
   * Enhanced token balances with Goldsky optimization
   */
  async getTokenBalances(address: string): Promise<TokenBalance[]> {
    try {
      // Try Goldsky first
      if (goldskyClient.isAvailable()) {
        const goldskyBalances = await goldskyClient.getTokenBalances(address)
        if (goldskyBalances.length > 0) {
          console.log(`✅ Goldsky found ${goldskyBalances.length} token balances`)
          return goldskyBalances
        }
      }
      ...
      console.error('Enhanced token balances error:', error)
      return await this.getRPCTokenBalances(address)
    }
  }

  /**
   * Get analytics data from Goldsky (not available via RPC)
   */
  async getPortfolioAnalytics(address: string) {
    if (!goldskyClient.isAvailable()) {
      console.log('Analytics require Goldsky - not available')
      return null
    }

    try {
      // This data is only available through Goldsky subgraph
      const client = goldskyClient['client'] // Access private client
      const query = `
      ...
      const data = await client.request(query, { userId: address.toLowerCase() })
      return data.user
    } catch (error) {
  ...
  /**
   * Get platform-wide statistics (Goldsky only)
   */
  async getPlatformStats() {
    if (!goldskyClient.isAvailable()) return null
    
    try {
      return await goldskyClient.getGlobalStats()
    } catch (error) {
  ...
  /**
   * Health check for both Goldsky and RPC
   */
  async healthCheck(): Promise<{ 
    goldsky: { isHealthy: boolean; latestBlock?: number }
    rpc: { isHealthy: boolean; latestBlock: number; chainId?: string }
  }> {
    const [goldskyHealth, rpcHealth] = await Promise.allSettled([
      goldskyClient.healthCheck(),
      this.getRPCHealthCheck()
    ])

    return {
      goldsky: goldskyHealth.status === 'fulfilled' ? goldskyHealth.value : { isHealthy: false },
      rpc: rpcHealth.status === 'fulfilled' ? rpcHealth.value : { isHealthy: false, latestBlock: 0 }
    }
  }

  // Your existing RPC methods (with minor renaming for clarity)
  private async getRPCPortfolioData(address: string): Promise<PortfolioData> {
    console.log(`Getting RPC portfolio data for ${address}`)
    
    // Get all data in parallel for better performance
    const [balanceData, transactions, tokens] = await Promise.all([
      this.getBalance(address),
      this.getRPCTransactionHistory(address, 50),
  ...
  private async getRPCTransactionHistory(address: string, limit = 20): Promise<Transaction[]> {
    try {
      console.log(`Getting RPC transaction history for ${address} (optimized)`)
...
// Interfaces for the RPC methods
